...

{
	...
	function install_settings()
	{
...

		{
			...
			$fields[$num]['label'] 			= __("Card","piereg");

			$fields[$num]['field_name'] 	= "card";			
...

	{
		...
		$structure["card"] = '<div class="fields_main"><div class="advance_options_fields"><input type="hidden" class="input_fields" name="field[%d%][type]"><div class="advance_fields"><label for="label_%d%">'.__("Label","piereg").'</label><input type="text" name="field[%d%][label]" id="label_%d%" value="Card" class="input_fields field_label"></div></div></div>';
...

	{
		...
		$keys 	= array("%user_login%","%user_email%","%blogname%","%siteurl%","%activationurl%","%firstname%","%lastname%","%user_url%","%user_aim%","%user_card%","%user_jabber%","%user_biographical_nfo%","%all_field%","%user_registration_date%","%reset_password_url%" ,"%invitation_code%","%pending_payment_url%","%blogname_url%","%user_ip%","%user_pass%");

		$values = array($user_login ,$user_email,$blog_name, $site_url,$activationurl,$first_name[0],$last_name[0],$user_url,$user_aim[0],$user_card[0],$user_jabber[0],$user_biographical_nfo[0], $all_field,$user_registration_date,$reset_password_url,$invitation_code,$pending_payment_url,$blogname_url,$user_ip,$user_pass);
